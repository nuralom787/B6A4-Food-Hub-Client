...
export default function ReviewCard({ review }: ReviewProps) {
    const renderStars = (rating: number) => {
        return [...Array(5)].map((_, index) => (
            <Star
                key={index}
                className={`w-4 h-4 ${index < rating ? "text-orange-500 fill-orange-500" : "text-gray-300"
                    }`}
            />
        ));
    };
    ...
    return (
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-all duration-300 relative flex flex-col h-full group">
            <Quote className="absolute top-4 right-6 w-8 h-8 text-orange-50 group-hover:text-orange-100 transition-colors" />
            <div className="flex gap-1 mb-4">
                {renderStars(review.rating)}
            </div>
            <p className="text-gray-600 italic leading-relaxed mb-6 grow">
